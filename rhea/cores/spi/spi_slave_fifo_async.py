...
def spi_slave_fifo_async(glbl, spibus, fifobus):
    ...
    tx_fifo_inst = fifo_async( fifobus.write_clock , clock , writepath, reset, size=fifosize)
    rx_fifo_inst = fifo_async( clock , fifobus.read_clock , readpath,reset, size=fifosize)
    mp_fifo_inst = fifobus.assign_read_write_paths(readpath, writepath)
